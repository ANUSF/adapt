...
jQuery.djtch = {
	...
	update: function(seed, html) {
		...
		newStuff.find('.dj-replace').each(function(i) {
			anchor(seed, this).replaceWith(content(this));
		}).end().find('.dj-append').each(function(i) {
			anchor(seed, this).append(content(this));
		}).end().find('.dj-prepend').each(function(i) {
			anchor(seed, this).prepend(content(this));
		}).end().find('.dj-after').each(function(i) {
			anchor(seed, this).after(content(this));
		}).end().find('.dj-before').each(function(i) {
			anchor(seed, this).before(content(this));
		});
...
jQuery.fn.extend({
	...
	djtchOnScrollLink: function() {
		if (this.length > 0) {
			...
			jQuery(window).unbind(ev, fn).bind(ev, this, fn).triggerHandler(ev);
		}
