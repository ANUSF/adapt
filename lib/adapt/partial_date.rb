...
class Adapt::PartialDate
  ...
  def initialize(*args)
    ...
    x = if args.size == 1
          case (val = args[0])
          when Date, Time, Adapt::PartialDate
            [val.year, val.month, val.day]
          when String
            Adapt::PartialDate::parse(val)
          when Integer, Year
            [val]
          else
  ...
  def compare_with(other_date)
    other = Adapt::PartialDate.new other_date
    this = [year, month, day].compact
    that = [other.year, other.month, other.day].compact
    n = [this.length, that.length].min
    this[0...n] <=> that[0...n]
  end
